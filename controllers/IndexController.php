        ...
        if (!isset($headers["Authorization"]) || empty($headers["Authorization"])) {
            ...
            if (!$user) {
                ...
                if ($block_expires > $now) {
                    ...
                    if ($user->authorised == 0) {
                        // is user authorized?
                        $this->buildErrorResponse(403, "login.USER_UNAUTHORIZED");
                    } else {
                        if (!password_verify($credentials['password'],
                            $user->password)) {
                            ...
                            if (!$user->save()) {
                                ...
                                $this->buildErrorResponse(400,
                                    "common.THERE_HAS_BEEN_AN_ERROR", $errors);
                            } else {
                                // checks if login_attempts are less or equal to 4
                                if ($user->login_attempts <= 4) {
                                    $this->buildErrorResponse(400,
                                        "login.WRONG_USER_PASSWORD");
                                } else {
                                    // adds 120 mins block
                                    ...
                                    if (!$user->save()) {
                                        ...
                                        $this->buildErrorResponse(400,
                                            "common.THERE_HAS_BEEN_AN_ERROR",
                                            $errors);
                                    } else {
                                        $this->buildErrorResponse(400,
                                            "login.TOO_MANY_FAILED_LOGIN_ATTEMPTS");
                                    }
                                }
                            }
                        } else {
                            ...
                            if (password_needs_rehash($user->password,
                                ...
                                $newHash = password_hash($credentials['password'],
                                    PASSWORD_DEFAULT, $options);
                            ...
                            $exp = strtotime("+" . $this->tokenConfig['expiration_time'] . " seconds",
                                $iat);
                            ...
                            if (!$user->save()) {
                                ...
                                if (!$newAccess->save()) {
                                    ...
                                    $this->buildSuccessResponse(200,
                                        "common.SUCCESSFUL_REQUEST", $data);
                                }
                            }
                        }
                    }
